...
const LoginPage = () => {
  ...
  async function handlerSubmit(e) {
    ...
    if (user.senha.trim().length >= 3 && user.email.trim().length >= 3) {
      //chamar a api
      try {
        const retorno = await api.post("/Login", {email: user.email, senha: user.senha}); //ou passar só "user" que é o objeto, ao invés de colocar as chaves do objeto, se o nome da chave e as props fossem diferentes, n teria como passar somente o "user", mas nesse caso daria
        ...
        alert('Usuário ou senha inválidos, ou conexão com a internet comprometida')
      }
    }
    }
          ...
          <form className="frm-login__formbox" onClick={handlerSubmit}>
            <Input
              ...
              manipulationFunction={(e) => {
                setUser({...user,   //VAI MEXER SOMENTE NO EMAIL, E DEIXANDO NO MESMO OBJECT, REST OPERATOR
            ...
            <Input
              ...
              value={user.senha}
              manipulationFunction={(e) => {
                setUser({...user,   //VAI MEXER SOMENTE NO SENHA, E DEIXANDO NO MESMO OBJECT, REST OPERATOR
